...
do_4b_dtb() {
# now create dts file
cat <<\EOF > /boot/bcm2711-rpi-4-b.dts && 
/dts-v1/;
...
if [ -f "$DTBFILE" ];then
	dtc -W no-dmas_property -W dmas_property -W no-unit_address_vs_reg -W no-unique_unit_address -W no-gpios_property -W no-avoid_unnecessary_addr_size -W no-clocks_property -I dtb -O dts -o /boot/tmp.dts $DTBFILE
	...
	if grep -q "brcm,pins = <0x8 0x7>;" /boot/tmp.dts; then
		if grep -q "brcm,function = <0x1>;" /boot/tmp.dts; then
			...
			dtc  -W no-dmas_property -W no-unit_address_vs_reg -W no-unique_unit_address -W no-gpios_property -W no-avoid_unnecessary_addr_size -W no-clocks_property  -I dts -O dtb -o $DTBFILE /boot/tmp.dts 
		fi
	fi
	rm /boot/tmp.dts
fi
...
if [ -f "$DTBFILE" ];then
	dtc  -W no-dmas_property -W no-unit_address_vs_reg -W no-unique_unit_address -W no-gpios_property -W no-avoid_unnecessary_addr_size -W no-clocks_property  -I dtb -O dts -o /boot/tmp.dts $DTBFILE
...
if [ -f "$DTBFILE" ];then
	dtc  -W dmas_property -W no-unit_address_vs_reg -W no-unique_unit_address -W no-gpios_property -W no-avoid_unnecessary_addr_size -W no-clocks_property  -I dtb -O dts -o /boot/tmp.dts $DTBFILE
	if grep -q "brcm,pins = <0x8 0x7>;" /boot/tmp.dts; then
		if grep -q "brcm,function = <0x1>;" /boot/tmp.dts; then
	      		echo "\n1B+";
			sed /boot/tmp.dts -i -e "s/brcm,function = <0x1>;/brcm,function = <0x6>;/"
