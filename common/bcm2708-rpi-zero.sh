...
do_0_dtb() {
# now create dts file
cat > /boot/bcm2708-rpi-zero.dts <<EOF
/dts-v1/;
...
EOF
dtc -W no-mboxes_property -W no-msi_parent_property -W no-phys_property -W no-resets_property -W no-thermal_sensors_property -W no-power_domains_property -W no-dmas_property -W no-unit_address_vs_reg -W no-unique_unit_address -W no-gpios_property -W no-avoid_unnecessary_addr_size -W no-clocks_property -I dts -O dtb -o /boot/bcm2708-rpi-zero.dtb /boot/bcm2708-rpi-zero.dts
}
